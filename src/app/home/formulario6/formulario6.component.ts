...
export class Formulario6Component implements OnInit {
  ...
  public id: number;
  public idUsers:any;
  @Output() questionResult5: EventEmitter<boolean> = new EventEmitter();
  constructor(
  ...
  ngOnInit(): void {
    this.initial()
    this.getTypesOfForms(4)
    this.getGeneralForms(this.idUsers.id, 4)
  ...
  getGeneralForms(item: number, iten: number){
    ...
    this._https.getGeneralForms(list).then((resulta: any)=>{
      ...
      if (this.menuItemsStore.length != 0) {
        ...
        this.questionResult5.emit(true)
      }
      this.alert.messagefin();
    }).catch((err: any)=>{
      console.log(err)
  ...
  downloadPDF() {
    this.alert.loading();
    // Extraemos el
    const DATA = document.getElementById('htmlDataCinco');
