...
export class Formulario5Component implements OnInit {
  ...
  @Output() questionResult4: EventEmitter<boolean> = new EventEmitter();
  constructor(
  ...
  ngOnInit(): void {
    this.initial()
    this.getTypesOfForms(3)
    this.getGeneralForms(this.idUsers.id, 3)
  ...
  getGeneralForms(item: number, iten: number){
    ...
    this._https.getGeneralForms(list).then((resulta: any)=>{
      ...
      if (this.menuItemsStore.length != 0) {
        ...
        console.log("entro")
        this.questionResult4.emit(true)
      }
      this.alert.messagefin();
    }).catch((err: any)=>{
      console.log(err)
  ...
  downloadPDF() {
    this.alert.loading();
    // Extraemos el
    const DATA = document.getElementById('htmlDataDos');
